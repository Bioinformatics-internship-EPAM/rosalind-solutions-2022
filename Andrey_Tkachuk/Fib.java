 ...
 * Link to Rosalind task: https://rosalind.info/problems/fib/
 * 
 ...
 * any given month will contain the rabbits that were alive the previous month, plus any 
 * new offspring. A key observation is that the number of offspring in any month is equal 
 * to the number of rabbits that were alive two months prior. As a result, if Fn 
 ...
 * Fn=Fn−1+Fn−2 (with F1=F2=1 to initiate the sequence). Although the sequence bears 
 * Fibonacci's name, it was known to Indian mathematicians over two millennia ago.
 * 
 * When finding the n-th term of a sequence defined by a recurrence relation, we can 
 ...
 * 
 * Return: The total number of rabbit pairs that will be present after n months, if we 
 *         begin with 1 pair and in each generation, every pair of reproduction-age rabbits 
 *         produces a litter of k rabbit pairs (instead of only 1 pair).
 */
public class Fib {

    static private String INPUT_FILE_NAME = "data/rosalind_fib.txt";
